#!/usr/bin/env python3
"""
CASS extractor (adjacent-type safe, heading filter, footer-date filter, paragraph reflow)

- Detect anchors in the LEFT gutter:  CASS <chapter>.<section>.<rule> [R/G/E/BG/C?]
- If the type token is adjacent (same line or tiny fallback line), record its x1 so body
  starts strictly to the RIGHT of it: body x0 > max(id_x1, type_x1) + body_margin
- Harvest body as LINES (not raw words), filter headings by font size/bold pattern,
  drop footers like 'September 2025', and reflow to readable paragraphs.
- Output list of rules (unique by (id,type)) sorted: 1, 1A, 3, 5, 6, 7, then the rest.
...
ID_TOKEN_RE = re.compile(rf"^{ID_CORE}(?P<trail>[A-Z]{{1,2}})?$")  # accepts glued 7.11.14R / 1.2.2BG
TYPE_ONLY_RE = re.compile(r"^(R|G|E|BG|C)$", re.I)
SECTION_RE = re.compile(r"^\s*Section\s*:\s*CASS\s+\d+[A-Z]?\.\d+", re.I)

# Things to drop from bodies
...
def looks_like_heading(ln: dict, heading_size_min: float) -> bool:
    ...
    if not t:
        return False
    # Skip lines that are obviously headings by size/bold
    if line_max_size(ln) >= heading_size_min or line_is_bold(ln):
        # Common chapter headings in body column start with R/G + Title
        if re.match(r"^[RG]\s+[A-Z][a-z]+", t):
            return True
...
def reflow_paragraphs_from_lines(lines: List[dict]) -> str:
    ...
    # collapse consecutive blanks later
    tmp: List[str] = []
    last_blank = False
    for s in cleaned:
        if not s.strip():
            ...
            if CASS_RE.match(t["text"]):
                ...
                while j < len(toks) and abs(toks[j]["doctop"] - t["doctop"]) <= y_tol:
                    ...
                    if m:
                        ...
                        typ = None
                        start_x = cand["x1"]  # start with ID x1
                        ...
                        if k < len(toks):
                            ...
                            if nxt["x0"] < left_limit and same_band and close_right and TYPE_ONLY_RE.match(nxt["text"]):
                                typ = nxt["text"].upper()
                                start_x = max(start_x, nxt["x1"])  # extend start barrier to the right edge of R/G
                        ...
                        anchors.append({
                            ...
                            "x1": start_x,      # << use the far-right edge of ID/Type cluster
                        })
                        break
                    j += 1
...
# ------------ body harvesting using LINES (with heading/date filters) ------------
...
def main():
    ...
    # geometry / detection
    ap.add_argument("--left-max-ratio", type=float, default=0.45, help="x ratio for left gutter max (anchors live here)")
    ...
    ap.add_argument("--body-margin", type=float, default=12.0, help="extra points to right of ID/Type cluster")
    ap.add_argument("--heading-size-min", type=float, default=11.5, help="minimum max font size to treat a line as heading")
